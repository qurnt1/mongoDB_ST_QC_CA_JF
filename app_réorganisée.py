...
# streamlit run app_réorganisée.py
########################################################################
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        # 4. Zone de réinitialisation complète (base + caches + état)
        st.subheader("🧨 Danger Zone")

        if st.button(
            "🗑️ DROP DB ET RESET CACHE",
            type="primary",
            use_container_width=True,
        ):
            try:
                # Suppression complète de la base MongoDB Paris2055
                client = pymongo.MongoClient(MONGO_URI)
                client.drop_database(MONGO_DB_NAME)
                client.close()

                # Réinitialisation des états associés aux résultats
                ...
                # Réinitialisation des états de migration et d'IA
                st.session_state["migration_logs"] = []
                st.session_state["migration_running"] = False
                st.session_state["migration_done_msg"] = ""
                st.session_state["ai_json_response"] = None
    ...
    # -----------------------------------------------------------------
    # CORPS PRINCIPAL : ONGLES FONCTIONNELS
    # -----------------------------------------------------------------
